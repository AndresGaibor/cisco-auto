# pt cmd real block probe

Fecha: Tue Apr 28 13:56:20 -05 2026
...
function isIosPrompt(v) {
  var line = trimLine(v);
  return /^[A-Za-z0-9._-]+(?:\\(config[^)]*\\))?[>#]\$/.test(line);
}

function lastNonEmptyLine(v) {
  var lines = normalizeEol(v).split(\"\\n\");
  for (var i = lines.length - 1; i >= 0; i--) {
    var s = trimLine(lines[i]);
    if (s) return s;
  }
  return \"\";
}
...
function lineContainsCommandEcho(line, command) {
  var rawLine = trimLine(line);
  var rawCommand = trimLine(command);
  ...
  var promptEchoPattern = new RegExp(
    \"^[A-Za-z0-9._-]+(?:\\(config[^)]*\\))?[>#]\\s*\" +
      escapeRegex(rawCommand) +
      \"\\s*\$\",
    \"i\"
  );
...
function isPagerOnlyLine(line) {
  return /^--More--\$/i.test(trimLine(line));
}

function outputLooksCompleteProbe(output, command) {
  var text = normalizeEol(output);
  var cmd = trimLine(command);
  var rawLines = text.split(\"\\n\");
  var lines = [];

  for (var i = 0; i < rawLines.length; i++) {
    var x = trimLine(rawLines[i]);
    if (x) lines.push(x);
  }

  var lastLine = lastNonEmptyLine(text);
  var echoLines = [];
  var promptLines = [];
  var pagerLines = [];
  var bodyLines = [];
  ...
  for (var j = 0; j < lines.length; j++) {
    var line = lines[j];
    var isEcho = lineContainsCommandEcho(line, cmd);
    var isPrompt = isIosPrompt(line);
    var isPager = isPagerOnlyLine(line);

    if (isEcho) echoLines.push({ index: j, line: line });
    if (isPrompt) promptLines.push({ index: j, line: line });
    if (isPager) pagerLines.push({ index: j, line: line });

    if (line && !isEcho && !isPrompt && !isPager) {
      bodyLines.push({ index: j, line: line });
    }
  }

  var hasPromptAtEnd = isIosPrompt(lastLine);
  var hasCommandEcho = cmd.length === 0 || echoLines.length > 0;
  var hasMeaningfulBody = bodyLines.length > 0;

  return {
    lineCount: lines.length,
    lastLine: lastLine,
    hasPromptAtEnd: hasPromptAtEnd,
    hasCommandEcho: hasCommandEcho,
    hasMeaningfulBody: hasMeaningfulBody,
    complete: hasCommandEcho && hasMeaningfulBody && hasPromptAtEnd,
    echoLines: echoLines.slice(0, 20),
    promptLines: promptLines.slice(-20),
    pagerLines: pagerLines.slice(0, 20),
    bodyFirst: bodyLines.slice(0, 12),
    bodyLast: bodyLines.slice(-12),
    firstLines: lines.slice(0, 25),
    lastLines: lines.slice(-25)
  };
}

function extractLatestCommandBlockProbe(output, command) {
  var text = normalizeEol(output);
  var cmd = trimLine(command);
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (
      lower === lowerCmd ||
      lower.indexOf(\">\" + lowerCmd) >= 0 ||
      lower.indexOf(\"#\" + lowerCmd) >= 0
    ) {
      matchingLines.push({ index: i, line: line });
    }
  }

  if (matchingLines.length > 0) {
    startIndex = matchingLines[matchingLines.length - 1].index;
  }

  var block = startIndex >= 0 ? lines.slice(startIndex).join(\"\\n\") : text;

  return {
    totalLines: lines.length,
    matchingLines: matchingLines,
    startIndex: startIndex,
    startLine: startIndex >= 0 ? trimLine(lines[startIndex]) : null,
    blockLen: block.length,
    blockFirst1200: block.slice(0, 1200),
    blockLast1200: block.slice(-1200),
    probe: outputLooksCompleteProbe(block, command)
  };
}

var output = safe(\"getOutput\");
var allOutput = safe(\"getAllOutput\");
var buffer = safe(\"getBuffer\");
var text = safe(\"getText\");
...
return JSON.stringify({
  ...
  candidates: {
    getOutput: { len: output.length, tail: output.slice(-800) },
    getAllOutput: { len: allOutput.length, tail: allOutput.slice(-800) },
    getBuffer: { len: buffer.length, tail: buffer.slice(-800) },
    getText: { len: text.length, tail: text.slice(-800) }
  },
  extraction: extractLatestCommandBlockProbe(output, \"show version\")
});
"
...
## latest deferred result with debug
```
...
}
{
  ...
  "debug": [
    ...
    "1777402154790 native-output-len=5830",
    "1777402154803 native-check command=\"show version\" prompt=\"SW-SRV-DIST>\" mode=\"user-exec\" blockLen=1432 complete=false blockTail=\"\\nHardware Board Revision Number  : 0x01\\n\\n\\nSwitch   Ports  Model              SW Version              SW Image\\n------   -----  -----              ----------              ----------\\n*    1   26     WS-C2960-24TT      12.2                    C2960-LANBASE-M\\n\\nConfiguration register is 0xF\\n\\n\\nSW-SRV-DIST>\"",
    "1777402154825 native-tick reason=reapStaleJobs phase=waiting-command waiting=true pending=set ageM
```
