...
export async function inspectCommandContext(
  ...
  const heartbeat = controller.getHeartbeatHealth?.() ?? { state: "unknown" as const };
  ...
  if (!summary.topologyMaterialized) {
    ...
    notes.push(
      `Topología en calentamiento: ${summary.deviceCount} dispositivos / ${summary.linkCount} enlaces.`,
    );
  }
