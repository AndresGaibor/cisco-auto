...
export const CHECK_META: CommandMeta = {
  ...
  examples: [
    ...
    { command: 'pt check lan-basic --fix', description: 'Validar y corregir problemas automáticamente' },
      ...
      const result = await runCommand<CheckResult>({
        ...
        execute: async (ctx) => {
          ...
          try {
            ...
            return createVerifiedResult('check', {
              scenario,
              passed,
              failed,
              warnings,
              ...
              executed: true,
              verified: failed === 0,
              checks: checks.map(c => ({ name: c.name, ok: c.status === 'pass' || c.status === 'warning', details: c.details })),
            });
          } finally {
...
async function validateLanBasic(ctx: any, _fix: boolean): Promise<CheckResultItem[]> {
  ...
  let devices: any[] = [];
  try {
    devices = await ctx.controller.listDevices();
  ...
  const switches = devices.filter(d => d.type === 'switch' || d.type === 'switch-l2' || d.type === 'switch_layer3' || d.type === 1);
  ...
  checks.push({ name: 'min-switches', status: 'pass', message: `${switches.length} switch(es) encontrado(s)` });
  ...
  const pc1Ip = (pc1Info as any).ip || pc1Info.ports?.find((p: any) => p.ipAddress && p.ipAddress !== '0.0.0.0')?.ipAddress;
  const pc1Mask = (pc1Info as any).mask || pc1Info.ports?.find((p: any) => p.ipAddress && p.ipAddress !== '0.0.0.0')?.subnetMask;
  const pc2Ip = (pc2Info as any).ip || pc2Info.ports?.find((p: any) => p.ipAddress && p.ipAddress !== '0.0.0.0')?.ipAddress;
  const pc2Mask = (pc2Info as any).mask || pc2Info.ports?.find((p: any) => p.ipAddress && p.ipAddress !== '0.0.0.0')?.subnetMask;

  if (!pc1Ip) {
    checks.push({ name: 'pc1-ip', status: 'fail', message: `${pc1.name} no tiene IP configurada`, fix: `pt config-host ${pc1.name} 192.168.10.10 255.255.255.0` });
  } else {
    checks.push({ name: 'pc1-ip', status: 'pass', message: `${pc1.name}: ${pc1Ip}/${pc1Mask}` });
  }

  if (!pc2Ip) {
    checks.push({ name: 'pc2-ip', status: 'fail', message: `${pc2.name} no tiene IP configurada`, fix: `pt config-host ${pc2.name} 192.168.10.20 255.255.255.0` });
  } else {
  ...
  if (pc1Ip && pc2Ip) {
    ...
    if (pc1Net === pc2Net) {
      checks.push({ name: 'same-subnet', status: 'pass', message: 'PC1 y PC2 están en la misma subred' });
    } else {
      checks.push({ name: 'same-subnet', status: 'fail', message: `PC1 (${pc1Net}) y PC2 (${pc2Net}) en subredes diferentes`, fix: 'Ajustar máscara o IP' });
    }

    const maskBits1 = subnetMaskToBits(pc1Mask);
    const maskBits2 = subnetMaskToBits(pc2Mask);
    if (maskBits1 !== maskBits2) {
      checks.push({ name: 'mask-mismatch', status: 'warning', message: `Máscaras diferentes: PC1 /${maskBits1}, PC2 /${maskBits2}`, details: { pc1Mask, pc2Mask }, fix: `Usar /${Math.min(maskBits1, maskBits2)} para ambos` });
    }
  }
  ...
  try {
    ...
    if (pc1IsPc) {
      ...
      const pingResult = await ctx.controller.execIos(pc1.name, `ping ${pc2Ip} repeat 4`, true, 10000);
      raw = pingResult.raw || '';
    }
    
    const success = raw.toLowerCase().includes('success') || raw.includes('Reply from') || (raw.includes('!') && !raw.includes('U')) || raw.includes('Paquetes perdidos: 0%') || raw.includes('0% packet loss') || (raw.includes('100%') === false && raw.includes('perdidos'));
    
    if (success) {
      checks.push({ name: 'ping-pc1-to-pc2', status: 'pass', message: `Ping ${pc1.name} → ${pc2.name} exitoso` });
    } else {
      checks.push({ name: 'ping-pc1-to-pc2', status: 'fail', message: `Ping ${pc1.name} → ${pc2.name} falló`, details: { raw: raw.slice(0, 200) }, fix: 'Verificar enlaces físicos y configuración IP' });
    }
  } catch (pingError: any) {
    checks.push({ name: 'ping-pc1-to-pc2', status: 'fail', message: `No se pudo ejecutar ping: ${pingError.message}`, fix: 'Verificar que PT esté corriendo' });
  }

  try {
    const switchInfo = await ctx.controller.inspectDevice(switch1.name);
    const ports = switchInfo.ports || [];
    const macsWithAddress = ports.filter((p: any) => p.macAddress && p.macAddress !== '0.0.0.0' && p.macAddress !== '0000.0000.0000');
    const uniqueMacs = new Set(macsWithAddress.map((p: any) => p.macAddress));

    if (uniqueMacs.size >= 2) {
      checks.push({ name: 'mac-table', status: 'pass', message: `${switch1.name} tiene ${uniqueMacs.size} MAC(s) en puertos`, details: { macCount: uniqueMacs.size } });
    } else {
      checks.push({ name: 'mac-table', status: 'warning', message: `${switch1.name} solo tiene ${uniqueMacs.size} MAC(s) en puertos - verificar enlaces`, details: { macCount: uniqueMacs.size, ports: ports.map((p: any) => ({ name: p.name, mac: p.macAddress })) } });
    }
  } catch (_macError: any) {
...
async function validateGateway(ctx: any, _fix: boolean): Promise<CheckResultItem[]> {
  ...
  for (const host of hosts) {
    ...
    if (!port?.ipAddress) {
      checks.push({ name: `gateway-${host.name}`, status: 'skip', message: `${host.name} no tiene IP` });
      continue;
    }
    checks.push({ name: `gateway-${host.name}`, status: 'pass', message: `${host.name}: ${port.ipAddress}` });
  }
...
function renderCheckResult(result: CheckResult, _fix: boolean): void {
  ...
  const summaryColor = result.failed > 0 ? chalk.red : result.warnings > 0 ? chalk.yellow : chalk.green;
