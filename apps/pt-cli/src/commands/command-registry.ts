...
export const PUBLIC_COMMAND_DEFINITIONS: PtCommandDefinition[] = [
  ...
  },
  {
    ...
    examples: [
      ...
      { command: "pt runtime status --live --json", description: "Ver estado vivo del runtime" },
      { command: "pt runtime reload", description: "Regenerar y pedir recarga del runtime" },
      { command: "pt runtime reload --json", description: "Solicitar hot reload y ver resultado" },
    ],
    related: ["pt doctor", "pt runtime logs", "pt runtime status --live --json"],
    agentHints: [
  ...
  },
  {
    ...
    description: "Comando universal de terminal. Detecta si el dispositivo es IOS o host y usa el backend correcto. Comandos de configuración se detectan automáticamente (no necesitas --config). Para lectura IOS explícita usa pt cmd read.",
    examples: [
      ...
      { command: 'pt cmd R1 "interface g0/0" "no shutdown"', description: "Configuración IOS detectada automáticamente" },
      { command: 'pt cmd read R1 "show running-config"', description: "Leer IOS sin preámbulo de paginador" },
    ],
    related: ["pt verify", "pt device list", "pt cmd history", "pt cmd read"],
    agentHints: [
      "Usa pt cmd para todo lo que se pueda escribir en consola.",
      "Usa pt cmd read para lecturas IOS largas o cuando quieres evitar depender del preámbulo de paginador.",
      "Usa --raw si quieres solo output del dispositivo.",
      "Usa --json para que el agente pueda parsear resultado y errores.",
      "--config es legacy: los comandos de configuración se detectan automáticamente ahora.",
    ],
