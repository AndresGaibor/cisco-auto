...
export function createDeviceListCommand(): Command {
  const cmd = new Command("list")
    ...
    .option("--ports", "Incluye puertos, sin conexiones live", false)
    .option("--links", "Incluye puertos y conexiones live", false)
    .option("--deep", "Inspección profunda: puertos, conexiones y metadata completa", false)
    .option("--no-deep", "No usar inspección profunda", false)
    ...
    .action(async (options, thisCmd) => {
      ...
      const ports = Boolean(options.ports);
