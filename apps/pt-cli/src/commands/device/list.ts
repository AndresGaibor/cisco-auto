...
export function createDeviceListCommand(): Command {
  /**
   * Lista dispositivos en Packet Tracer.
   * Usa runCommand() para manejo automático del ciclo de vida del controller.
   */
    ...
    .action(async (options, thisCmd) => {
      ...
      const result = await runCommand<{
        ...
        execute: async ({ controller }) => {
          ...
          const snapshotDevices = Object.values(snapshot?.devices ?? {});

          const devices = snapshotDevices.map((d) => ({
            name: d.name,
            model: d.model,
            type: d.type,
            power: d.power,
          ...
          return createSuccessResult("device.list", {
            devices: filtered,
      ...
      const totalLinks =
        Object.keys(deviceLinks).reduce((sum, key) => sum + (deviceLinks[key]?.length || 0), 0) / 2;
