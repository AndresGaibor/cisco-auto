...
function extractLinkEndpoints(link: any): { device1: string; port1: string; device2: string; port2: string } {
  return {
    device1: link?.device1 || link?.sourceDeviceId || link?.source?.deviceId || link?.source || 'unknown',
    port1: link?.port1 || link?.sourcePort || link?.source?.port || '',
    device2: link?.device2 || link?.targetDeviceId || link?.target?.deviceId || link?.target || 'unknown',
...
export async function runInspectTopology(options: { device?: string; json?: boolean; deprecationLabel?: string } = {}): Promise<void> {
  ...
  try {
    ...
    if (result.links.length === 0) {
      ...
      for (const link of result.links) {
        console.log(
          '  ' +
            chalk.cyan(link.device1) + chalk.gray(`.${link.port1}`) +
            chalk.yellow(' ↔ ') +
            chalk.cyan(link.device2) + chalk.gray(`.${link.port2}`),
        );
      }
    }
