...
const LAB_LIFT_META: CommandMeta = {
  ...
  longDescription: 'Crea la topología solicitada con un CORE3650, cuatro switches 2960, cuatro PCs y un servidor, y aplica VLANs, Rapid-PVST+, trunks, SVIs, DHCPv4 y direccionamiento base.',
  examples: [
    {
...
export function buildScenarioPlan(): LiftScenarioPlan {
  ...
  const links: LiftLinkSpec[] = [
    ...
    { fromDevice: CORE_NAME, fromPort: CORE_SERVER_PORT, toDevice: SERVER_NAME, toPort: 'FastEthernet0', cableType: 'straight' },
  ];
...
function buildAccessSwitchCommands(name: string, accessVlan: number, managementSuffix: number): string[] {
  return [
    'configure terminal',
    `hostname ${name}`,
    'spanning-tree mode rapid-pvst',
...
async function waitForTopologyMaterialization(controller: {
  snapshot: () => Promise<{ devices: Record<string, unknown>; links: Record<string, unknown> }>;
  listDevices: () => Promise<Array<{ name: string }>>;
}, expectedDeviceNames: string[], expectedLinkCount: number, maxAttempts = 20, delayMs = 500): Promise<{ devices: Array<{ name: string }>; snapshot: { devices: Record<string, unknown>; links: Record<string, unknown> } | null }> {
...
function summarizeChecks(snapshot: {
  ...
  return [
    ...
    {
      name: 'core.vlans',
      ok: [VLAN_USERS, VLAN_ADMIN, VLAN_SERVERS, VLAN_MGMT].every((vlan) => snapshot.coreVlans.includes(vlan)),
      details: { expected: [VLAN_USERS, VLAN_ADMIN, VLAN_SERVERS, VLAN_MGMT], actual: snapshot.coreVlans },
    },
    {
      name: 'core.svis-up',
      ok: ['Vlan10', 'Vlan20', 'Vlan30', 'Vlan99'].every((iface) => snapshot.coreInterfacesUp.some((entry) => entry.toLowerCase().includes(iface.toLowerCase()))),
    ...
    },
    {
      ...
      ok: snapshot.mgmtSwitches.every((item) => typeof item.ip === 'string' && item.ip.startsWith('192.168.99.')),
...
export function createLabLiftCommand(): Command {
  const cmd = new Command('lift')
    ...
    .action(async function (this: Command) {
      ...
      if (globalExamples) {
        console.log(formatExamples(getExamples('lab lift')) + formatRelatedCommands(getRelatedCommands('lab lift')));
        return;
      }
        ...
        try {
          await ctx.logPhase('prepare', { scenario: SCENARIO_NAME });
            ...
            try {
              ...
              await ctx.logPhase('warn', { message: `No se pudo limpiar la topología previa: ${message}` });
            }
            ...
            for (const device of plan.devices) {
              await ctx.controller.addDevice(device.name, device.model, { x: device.x, y: device.y });
            }

            const deviceNamesReady = await waitForDeviceNames(ctx.controller, plan.devices.map((device) => device.name));
            if (!deviceNamesReady) {
            ...
            for (const link of plan.links) {
              await ctx.controller.addLink(link.fromDevice, link.fromPort, link.toDevice, link.toPort, link.cableType);
            }
            ...
            const checks = summarizeChecks({
              ...
              coreInterfacesUp: coreInterfaces.interfaces.map((iface) => `${iface.interface} ${iface.status}/${iface.protocol}`),
            ...
            if (verified) {
              return createVerifiedResult('lab.lift', resultData, {
                ...
                checks,
                warnings: [...plan.notes],
                verificationSource: ['snapshot', 'show running-config', 'show vlan', 'show ip interface brief', 'inspect'],
              }) as CliResult<LiftResult>;
            }
            ...
            return createVerifiedResult('lab.lift', resultData, {
              ...
              checks,
              warnings: [...plan.notes, 'La automatización de servicios DNS/WEB/EMAIL del Server-PT no está expuesta por la API actual.'],
              verificationSource: ['snapshot', 'show running-config', 'show vlan', 'show ip interface brief', 'inspect'],
            }) as CliResult<LiftResult>;
          } catch (error) {
            return createErrorResult('lab.lift', {
