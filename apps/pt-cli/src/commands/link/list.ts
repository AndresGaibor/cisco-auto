...
export function createLinkListCommand(): Command {
  const cmd = new Command('list')
    ...
    .action(async (options) => {
      const result = await runCommand<{ devices: string[]; links: Array<{ id: string; device1: string; port1: string; device2: string; port2: string; cableType: string }> }>({
        action: 'link.list',
        meta: { id: 'link.list', summary: 'Listar conexiones', examples: [], related: [] },
        flags: { json: Boolean(options.json), jq: null, output: 'table', verbose: false, quiet: false, trace: false, tracePayload: false, traceResult: false, traceDir: null, traceBundle: false, traceBundlePath: null, sessionId: null, examples: false, schema: false, explain: false, plan: false, verify: false },
        execute: async ({ controller }) => {
          ...
          const allLinks: Array<{ id: string; device1: string; port1: string; device2: string; port2: string; cableType: string }> = [];
            ...
            try {
              ...
              for (const [id, link] of Object.entries(linksData)) {
                ...
                if (deviceNames.has(d1) && deviceNames.has(d2)) {
                  ...
                  const isDuplicate = allLinks.some(l => 
          ...
          return createSuccessResult('link.list', { links: allLinks, devices: Array.from(deviceNames) });
        },
      });
      ...
      for (const link of links) {
        ...
        console.log(`${chalk.cyan(link.device1)}:${link.port1} ${chalk.gray('↔')} ${chalk.cyan(link.device2)}:${link.port2}`);
