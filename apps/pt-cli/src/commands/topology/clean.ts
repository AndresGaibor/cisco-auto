...
export const TOPOLOGY_CLEAN_META: CommandMeta = {
  ...
  longDescription: 'Elimina dispositivos del canvas de Packet Tracer. Soporta eliminación individual, por tipo, o completa.',
  examples: [
    {
...
export function createTopologyCleanCommand(): Command {
  const cmd = new Command('clean')
    ...
    .option('-t, --type <type>', 'Eliminar todos los dispositivos de un tipo (router, switch, pc, server)')
        ...
        execute: async (ctx): Promise<CliResult<TopologyCleanResult>> => {
          ...
          try {
            ...
            return createSuccessResult('topology.clean', {
              ...
              advice: [
                'Ejecuta bun run pt device list para verificar los cambios',
              ],
            });
          } finally {
