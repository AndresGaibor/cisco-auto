...
export class DeviceCatalog {
     ...
     */
    getByModel(model) {
        return allDevices.find(d => d.model.toLowerCase() === model.toLowerCase());
    }
    /**
    ...
    search(query) {
        ...
        // Filtrar por vendor
        if (query.vendor) {
            results = results.filter(d => d.vendor.toLowerCase() === query.vendor.toLowerCase());
        }
        // Filtrar por serie
        if (query.series) {
            results = results.filter(d => d.series.toLowerCase().includes(query.series.toLowerCase()));
        }
        // Búsqueda de texto libre
        if (query.search) {
            const searchLower = query.search.toLowerCase();
            results = results.filter(d => d.model.toLowerCase().includes(searchLower) ||
        ...
        if (query.hasCapability && query.capabilityValue !== undefined) {
            results = results.filter(d => {
                ...
                if (Array.isArray(capability)) {
                    return query.capabilityValue.every(v => capability.includes(v));
                }
                return false;
            });
        }
        ...
        if (query.minPorts) {
            results = results.filter(d => getTotalPorts(d.fixedPorts) >= query.minPorts);
        }
        // Filtrar por soporte de módulos
        if (query.supportsModules !== undefined) {
            results = results.filter(d => d.capabilities.supportsModules === query.supportsModules);
        }
        // Filtrar por tags
        if (query.tags && query.tags.length > 0) {
            results = results.filter(d => query.tags.some(tag => d.tags.some(t => t.toLowerCase() === tag.toLowerCase())));
        }
     ...
     */
    getSecurityDevices() {
        return allDevices.filter(d => d.type === 'firewall' || d.capabilities.supportsFirewall);
    }
    /**
     * Obtiene dispositivos industriales
     */
    getIndustrialDevices() {
        return this.search({
    ...
    getCCNADevices() {
        return this.search({
            ...
            includeLegacy: true,
            includeGeneric: true
        }).filter(d => d.model.includes('2960') ||
     ...
     */
    areCompatible(device1Id, device2Id) {
...
export function resetDefaultCatalog() {
    defaultCatalog = null;
}
