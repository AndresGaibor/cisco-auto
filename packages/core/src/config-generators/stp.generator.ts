...
import type { STPSpec, STPVlanConfig, STPInterfaceConfig } from '../canonical/protocol.spec';
...
export class STPGenerator {
   ...
   */
  private static generateVlanConfig(config: STPVlanConfig): string[] {
    const commands: string[] = [];
   ...
   */
  static validate(spec: STPSpec): { valid: boolean; errors: string[]; warnings: string[] } {
    const errors: string[] = [];
