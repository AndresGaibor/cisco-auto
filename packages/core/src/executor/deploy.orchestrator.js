 ...
 */
function deviceSpecToDevice(spec, credentials) {
    return {
        name: spec.name,
        type: spec.type,
...
export class DeployOrchestrator {
     ...
     */
    async deployLab(lab, getCredentials) {
        ...
        for (const batch of plan.order) {
            ...
            const batchResults = await Promise.all(batchDevices.map(device => this.deployDevice(device, getCredentials(device))));
     ...
     */
    async deployDevice(device, credentials) {
        const startTime = new Date();
        const commands = [];
        const errors = [];
        const warnings = [];
        ...
        try {
            ...
            if (this.options.autoRollback && this.options.backupDir) {
                try {
                    ...
                    const rollbackErr = rollbackError;
                    errors.push({
     ...
     */
    async saveBackup(connector, deviceName) {
    ...
    async rollback(connector, deviceName) {
        ...
        if (deviceBackups.length > 0) {
            ...
            const lines = backupConfig.split('\n').filter((l) => !l.startsWith('!') && l.trim() !== '' && !l.startsWith('end'));
 ...
 */
export async function deployToDevice(device, credentials, options = {}) {
