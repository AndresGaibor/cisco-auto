...
export class ValidationExecutor {
     ...
     */
    async validatePing(connector, destination, expected) {
        try {
     ...
     */
    async validateInterface(connector, interfaceName, expectedUp) {
        try {
     ...
     */
    async validateVlan(connector, vlanId, expectedName) {
        try {
            ...
            const actualName = match ? (match[1] ?? 'unknown') : 'not found';
            let passed = exists;
     ...
     */
    async validateRoute(connector, destination) {
        try {
     ...
     */
    async validateAll(specs, concurrency = 5) {
        ...
        for (let i = 0; i < specs.length; i += concurrency) {
            ...
            const batchResults = await Promise.all(batch.map(async (spec) => {
                ...
                return [spec.device.name, result];
            }));
 ...
 */
export function generateValidationSpec(device, credentials) {
    const checks = {};
