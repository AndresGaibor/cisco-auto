 ...
 */
function calculateComplexity(totalDevices) {
    if (totalDevices < 10)
        return 'low';
    if (totalDevices <= 20)
        return 'medium';
 ...
 */
function calculateCost(routerCount, switchCount, pcCount, serverCount) {
 ...
 */
function calculateTime(routerCount, switchCount, pcCount, serverCount) {
 ...
 */
function generateBreakdown(routerCount, switchCount, pcCount, serverCount) {
    const breakdown = [];
    if (routerCount > 0) {
        breakdown.push({
...
export const ptEstimatePlanTool = {
    ...
    handler: async (input) => {
        const { routerCount, switchCount, pcCount, serverCount, networkType } = input;
        // Validar entrada
...
//# sourceMappingURL=estimate-plan.js.map