...
function isIpInUse(ip, usedIps) {
    return usedIps.some(u => u.ip === ip);
}
function getNextAvailableIp(baseIp, usedIps) {
...
function getCorrectCableType(deviceType1, deviceType2) {
...
function getNextAvailableSubnet(usedSubnets, startSubnet = '192.168.1.0') {
...
function applyFixesToPlan(plan) {
    const appliedFixes = [];
...
function validateWithoutFixes(plan) {
    const errors = [];
    for (const device of plan.devices) {
        if (!modelExistsInCatalog(device.model.name)) {
            errors.push({
