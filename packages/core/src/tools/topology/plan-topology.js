...
function getNextIp(baseIp, offset) {
    return longToIp(ipToLong(baseIp) + offset);
}
function selectDeviceModel(type) {
...
function generateDevices(params) {
    const devices = [];
    let ipOffset = 1;
...
export const ptPlanTopologyTool = {
    ...
    handler: async (input) => {
        const params = {
            routerCount: input.routerCount ?? 1,
            switchCount: input.switchCount ?? 1,
            pcCount: input.pcCount ?? 2,
            serverCount: input.serverCount ?? 0,
            networkType: input.networkType ?? 'single_lan',
            routingProtocol: input.routingProtocol ?? 'static',
            dhcpEnabled: input.dhcpEnabled ?? true,
            baseNetwork: input.baseNetwork ?? '192.168.1.0',
            subnetMask: input.subnetMask ?? '255.255.255.0'
        };
...
//# sourceMappingURL=plan-topology.js.map