...
function buildValidatePlanError(message, code) {
    return {
        ok: false,
        error: message,
        code,
 ...
 */
function portExistsOnModel(modelName, portName) {
 ...
 */
function getAllConfiguredIps(plan) {
    const ips = [];
 ...
 */
function findDuplicateIps(plan) {
    const ipMap = new Map();
    for (const device of plan.devices) {
        for (const iface of device.interfaces) {
            if (iface.ip && iface.configured) {
 ...
 */
const CABLE_PORT_COMPATIBILITY = {
 ...
 */
function isCableCompatibleWithPort(cableType, portType) {
 ...
 */
function getPortType(modelName, portName) {
    const device = deviceCatalog.find(d => d.name === modelName);
    if (!device)
        return null;
 ...
 */
function validateTopologyPlan(plan) {
    const errors = [];
    const warnings = [];
    // ============================================
...
export const ptValidatePlanTool = {
