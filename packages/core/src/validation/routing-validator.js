...
export class RoutingValidator {
    ...
    validateOSPF(ospf) {
        ...
        if (!Number.isInteger(ospf.processId) || ospf.processId < 1 || ospf.processId > 65535) {
            errors.push(createError(ValidationCodes.OSPF_INVALID_PID, `Invalid OSPF process ID: ${ospf.processId} (1-65535)`, 'routing.ospf.processId'));
        }
        // Validate areas
        if (ospf.areas && ospf.areas.length > 0) {
            for (const area of ospf.areas) {
                if (!this.isValidAreaId(area.areaId)) {
                    errors.push(createError(ValidationCodes.OSPF_INVALID_AREA, `Invalid OSPF area: ${area.areaId}`, 'routing.ospf.areas'));
                }
                if (area.type && !this.isValidAreaType(area.type)) {
                    errors.push(createError(ValidationCodes.OSPF_INVALID_AREA, `Invalid OSPF area type: ${area.type}`, 'routing.ospf.areas'));
                }
            }
        }
        return createValidationResult(errors, warnings);
    ...
    validateEIGRP(eigrp) {
        ...
        if (!Number.isInteger(eigrp.asNumber) || eigrp.asNumber < 1 || eigrp.asNumber > 65535) {
            errors.push(createError(ValidationCodes.EIGRP_INVALID_AS, `Invalid EIGRP AS: ${eigrp.asNumber} (1-65535)`, 'routing.eigrp.asNumber'));
        }
        return createValidationResult(errors, warnings);
    }
    validateBGP(bgp) {
        ...
        if (!Number.isInteger(bgp.asn) || bgp.asn < 1 || bgp.asn > 4294967295) {
            errors.push(createError(ValidationCodes.BGP_INVALID_ASN, `Invalid BGP AS: ${bgp.asn} (1-4294967295)`, 'routing.bgp.asn'));
        }
        // Validate router ID format if present
        if (bgp.routerId && !this.isValidIpAddress(bgp.routerId)) {
            errors.push(createError(ValidationCodes.BGP_INVALID_ROUTEID, `Invalid BGP router ID: ${bgp.routerId}`, 'routing.bgp.routerId'));
        }
        // Validate neighbors
        if (bgp.neighbors && bgp.neighbors.length > 0) {
            for (const neighbor of bgp.neighbors) {
                if (!this.isValidIpAddress(neighbor.ip)) {
                    errors.push(createError(ValidationCodes.INVALID_INPUT, `Invalid BGP neighbor IP: ${neighbor.ip}`, 'routing.bgp.neighbors'));
                }
                if (!Number.isInteger(neighbor.remoteAs) || neighbor.remoteAs < 1 || neighbor.remoteAs > 4294967295) {
                    errors.push(createError(ValidationCodes.BGP_INVALID_ASN, `Invalid BGP neighbor AS: ${neighbor.remoteAs}`, 'routing.bgp.neighbors'));
                }
            }
        }
        return createValidationResult(errors, warnings);
    }
    validateStaticRoute(route) {
        const errors = [];
        if (!this.isValidNetwork(route.network)) {
            errors.push(createError(ValidationCodes.ROUTING_INVALID_NETWORK, `Invalid network: ${route.network}`, 'routing.staticRoutes'));
        }
        if (route.metric !== undefined) {
            if (!Number.isInteger(route.metric) || route.metric < 1 || route.metric > 4294967295) {
                errors.push(createError(ValidationCodes.ROUTING_INVALID_METRIC, `Invalid metric: ${route.metric}`, 'routing.staticRoutes'));
            }
        }
        return errors;
    }
...
//# sourceMappingURL=routing-validator.js.map