...
describe('CommandProcessor + CrashRecovery contract', () => {
  ...
  beforeEach(() => {
    ...
    backpressure = new BackpressureManager(paths, { maxPending: 2, checkIntervalMs: 5, maxWaitMs: 50 });
  ...
  test('preserves result identity across restart and recovery', () => {
    writeFileSync(join(paths.commandsDir(), '000000000001-configIos.json'), JSON.stringify(commandEnvelope(1, 'configIos', { device: 'R1' }), null, 2));
    writeFileSync(join(paths.commandsDir(), '000000000002-configIos.json'), JSON.stringify(commandEnvelope(2, 'configIos', { device: 'R2' }), null, 2));
