import { detectModeFromPrompt, isHostMode, normalizePrompt } from "../prompt/detector.js";
import type { TerminalSessionState, TerminalSessionKind, TerminalMode } from "../session/state.js";
...
export function checkIsCommandFinished(
    ...
    if (iosPromptMatch || /[>#]$/.test(prompt)) {
        if (/[A-Z]:\\>$/i.test(prompt)) {
            ...
            // REGLA DE ORO IOS: No terminar si el comando ni siquiera ha empezazco a procesarse
