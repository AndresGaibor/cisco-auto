...
export class IosSemanticService {
  ...
  async configureSvi(
    ...
    await this.execution.configIos(
      device,
      plan.steps.map((step: any) => step.command),
      { save: options?.save },
    );
    try {
  ...
  async configureDhcpPool(
    ...
    const verification = await this.execution.execInteractive(device, "show running-config", {
      parse: false,
  ...
  async configureOspfNetwork(
    ...
    const verification = await this.execution.execInteractive(device, "show ip protocols", {
      parse: false,
      ensurePrivileged: true,
      timeout: 10000,
    });
  ...
  async configureAccessListStandard(
    ...
    await this.execution.configIos(device, commands, { save: options?.save ?? true });
    const verification = await this.execution.execInteractive(device, "show access-lists", {
      parse: false,
      ensurePrivileged: true,
      timeout: 10000,
    });
    ...
    try {
      ...
      switch (checkName) {
        ...
        case "access-port":
          ...
          verification = await this.verifier.verifyAccessPort(
            device,
            first,
            second ? parseInt(second) : undefined,
          );
          break;
        case "trunk-port":
          if (!first) return "unverified";
          verification = await this.verifier.verifyTrunkPort(
            device,
            first,
            rest.slice(1).filter(Boolean).map(Number),
          );
          break;
        case "static-route":
        ...
        case "dhcp-pool":
          ...
          break;
        case "ospf":
          verification = await this.verifier.verifyOspf(
            device,
            first ? parseInt(first) : undefined,
          );
          break;
        case "acl":
          if (!first) return "unverified";
