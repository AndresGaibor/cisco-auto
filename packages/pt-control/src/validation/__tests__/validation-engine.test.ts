...
import { strictPolicy } from "../policies.js";
...
describe("Issue #7: ValidationEngine caching", () => {
  test("returns cached result for same context within TTL", () => {
    ...
    expect(rule.validate).toHaveBeenCalledTimes(1); // Should be cached (not called again)
...
describe("Policy integration", () => {
  ...
  test("blocked is false when no diagnostics", () => {
    ...
    // Use strictPolicy which blocks on error or blocking=true, but no diagnostics means no block
