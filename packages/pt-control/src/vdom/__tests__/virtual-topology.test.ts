...
describe("Issue #6: calculateDeltaFrom efficiency", () => {
  ...
  test("notifies handlers on changes", () => {
    ...
    const delta = receivedDelta as { devices: Array<{ op: string; device?: { name: string } }> };
    expect(delta.devices.length).toBe(1);
    expect(delta.devices[0]!.op).toBe("add");
    expect(delta.devices[0]!.device?.name).toBe("Router1");
...
describe("replaceSnapshot behavior", () => {
  test("replaces snapshot and increments version", () => {
    ...
    // Note: replaceSnapshot merges with current snapshot, not full replacement
  });
...
describe("Handler management", () => {
  ...
  test("unsubscribe is idempotent", () => {
    ...
    // First unsubscribe removes the handler
    unsubscribe();

    // Subsequent calls should not throw (idempotent behavior)
    unsubscribe();
    unsubscribe();
    ...
    // Handler was removed, so count should be 0
    expect(count).toBe(0);
  });
  ...
  test("handlers array cleanup after unsubscribe", () => {
    ...
    let callCount = 0;
    const unsubscribe1 = topology.onChange(() => { callCount++; });
    const unsubscribe2 = topology.onChange(() => { callCount++; });

    // Trigger an event - both handlers should fire
    topology.applyEvent({
      type: "device-added",
      name: "Router1",
      model: "ISR 2911",
    ...
    // Trigger another event - only second handler should fire
    topology.applyEvent({
      type: "device-added",
      name: "Router2",
      model: "2960",
