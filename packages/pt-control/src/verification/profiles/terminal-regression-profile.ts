// ============================================================================
// Terminal Regression Profile - Smoke tests para terminal IOS y host
// ============================================================================

export interface TerminalRegressionCase {
  id: string;
  description: string;
  device: string;
  deviceKind: "ios" | "host";
  command: string;
  preCommands?: string[];
  postCommands?: string[];
  expectedOk: boolean;
  tags: string[];
  expectedPromptPattern?: string;
  expectedModeAfter?: string;
  expectedOutputIncludes?: string[];
  expectedErrorCode?: string;
  expectedErrorIncludes?: string[];
  maxDurationMs: number;
}

export const terminalRegressionCases: TerminalRegressionCase[] = [
  // ========================================================================
  // IOS básico
  // ========================================================================
  {
    id: "ios-show-clock",
    description: "show clock responde con hora IOS",
    device: "ABEJITA",
    deviceKind: "ios",
    preCommands: ["enable"],
    command: "show clock",
    expectedOk: true,
    tags: ["ios", "show", "short-output"],
    expectedPromptPattern: "#",
    expectedModeAfter: "privileged-exec",
    expectedOutputIncludes: ["show clock", "UTC"],
    postCommands: ["enable"],
    maxDurationMs: 10000,
  },
  {
    id: "ios-show-version",
    description: "show version muestra info del dispositivo",
    device: "ABEJITA",
    deviceKind: "ios",
    preCommands: ["enable"],
    command: "show version",
    expectedOk: true,
    tags: ["ios", "show"],
    expectedOutputIncludes: ["Cisco IOS Software"],
    maxDurationMs: 10000,
  },
  {
    id: "ios-show-privilege",
    description: "show privilege preserva el modo actual y muestra nivel",
    device: "ABEJITA",
    deviceKind: "ios",
    preCommands: ["enable"],
    command: "show privilege",
    expectedOk: true,
    tags: ["ios", "show", "privilege"],
    expectedOutputIncludes: ["privilege level"],
    maxDurationMs: 10000,
  },
  {
    id: "ios-enable",
    description: "enable entra a privileged exec",
    device: "ABEJITA",
    deviceKind: "ios",
    preCommands: [],
    command: "enable",
    expectedOk: true,
    tags: ["ios", "mode"],
    expectedPromptPattern: "#",
    expectedModeAfter: "privileged-exec",
    maxDurationMs: 10000,
  },
  {
    id: "ios-disable",
    description: "disable vuelve a user exec",
    device: "ABEJITA",
    deviceKind: "ios",
    preCommands: ["enable", "disable"],
    command: "disable",
    expectedOk: true,
    tags: ["ios", "mode"],
    expectedPromptPattern: ">",
    expectedModeAfter: "user-exec",
    maxDurationMs: 10000,
  },
  {
    id: "ios-config-t",
    description: "conf t entra a configure terminal",
    device: "ABEJITA",
    deviceKind: "ios",
    preCommands: ["enable"],
    command: "conf t",
    expectedOk: true,
    tags: ["ios", "config", "mode"],
    expectedPromptPattern: "(config)#",
    expectedModeAfter: "global-config",
    postCommands: ["end"],
    maxDurationMs: 10000,
  },
  {
    id: "ios-interface",
    description: "interface fa0/1 entra a config-if",
    device: "ABEJITA",
    deviceKind: "ios",
    preCommands: ["enable", "conf t"],
    command: "interface fa0/1",
    expectedOk: true,
    tags: ["ios", "config", "submode"],
    expectedPromptPattern: "(config-if)#",
    expectedModeAfter: "config-if",
    postCommands: ["end"],
    maxDurationMs: 10000,
  },
  {
    id: "ios-exit",
    description: "exit desde config-if vuelve a global-config",
    device: "ABEJITA",
    deviceKind: "ios",
    preCommands: ["enable", "conf t", "interface fa0/1"],
    command: "exit",
    expectedOk: true,
    tags: ["ios", "config", "mode"],
    expectedPromptPattern: "(config)#",
    expectedModeAfter: "global-config",
    postCommands: ["end"],
    maxDurationMs: 10000,
  },
  {
    id: "ios-end",
    description: "end desde config-if vuelve a privileged exec",
    device: "ABEJITA",
    deviceKind: "ios",
    preCommands: ["enable", "conf t", "interface fa0/1"],
    command: "end",
    expectedOk: true,
    tags: ["ios", "config", "mode"],
    expectedPromptPattern: "#",
    expectedModeAfter: "privileged-exec",
    maxDurationMs: 10000,
  },

  // ========================================================================
  // Pager (casos críticos)
  // ========================================================================
  {
    id: "ios-show-running-config",
    description: "show running-config con paginacion automatica",
    device: "ABEJITA",
    deviceKind: "ios",
    preCommands: ["enable"],
    command: "show running-config",
    expectedOk: true,
    tags: ["ios", "show", "pager", "long-output"],
    expectedPromptPattern: "#",
    expectedModeAfter: "privileged-exec",
    expectedOutputIncludes: ["Building configuration", "Current configuration"],
    maxDurationMs: 70000,
  },
  {
    id: "ios-show-interfaces",
    description: "show interfaces con paginacion",
    device: "ABEJITA",
    deviceKind: "ios",
    preCommands: ["enable"],
    command: "show interfaces",
    expectedOk: true,
    tags: ["ios", "show", "pager"],
    expectedOutputIncludes: ["FastEthernet0/1"],
    maxDurationMs: 70000,
  },
  {
    id: "ios-show-spanning-tree",
    description: "show spanning-tree con paginacion",
    device: "ABEJITA",
    deviceKind: "ios",
    preCommands: ["enable"],
    command: "show spanning-tree",
    expectedOk: true,
    tags: ["ios", "show", "pager"],
    expectedOutputIncludes: ["Spanning Tree"],
    maxDurationMs: 70000,
  },
  {
    id: "ios-show-mac-address-table",
    description: "show mac address-table dynamic",
    device: "ABEJITA",
    deviceKind: "ios",
    preCommands: ["enable"],
    command: "show mac address-table dynamic",
    expectedOk: true,
    tags: ["ios", "show", "pager"],
    expectedOutputIncludes: ["MAC Address"],
    maxDurationMs: 70000,
  },

  // ========================================================================
  // Confirm prompts
  // ========================================================================
  {
    id: "ios-copy-running-config",
    description: "copy running-config startup-config con confirmacion",
    device: "ABEJITA",
    deviceKind: "ios",
    preCommands: ["enable"],
    command: "copy running-config startup-config",
    expectedOk: true,
    tags: ["ios", "config", "confirm"],
    expectedOutputIncludes: ["Destination filename"],
    maxDurationMs: 30000,
  },
  {
    id: "ios-write-memory",
    description: "write memory con confirmacion",
    device: "ABEJITA",
    deviceKind: "ios",
    preCommands: ["enable"],
    command: "write memory",
    expectedOk: true,
    tags: ["ios", "config", "confirm"],
    expectedOutputIncludes: ["Building configuration"],
    maxDurationMs: 30000,
  },

  // ========================================================================
  // Errores semánticos
  // ========================================================================
  {
    id: "ios-invalid-input",
    description: "comando invalido genera error IOS",
    device: "ABEJITA",
    deviceKind: "ios",
    preCommands: ["enable"],
    command: "show esto-no-existe",
    expectedOk: false,
    tags: ["ios", "semantic-error"],
    expectedErrorCode: "IOS_INVALID_INPUT",
    expectedErrorIncludes: ["% Invalid input detected"],
    postCommands: [],
    maxDurationMs: 10000,
  },
  {
    id: "ios-incomplete",
    description: "comando incompleto genera error",
    device: "ABEJITA",
    deviceKind: "ios",
    preCommands: ["enable"],
    command: "show",
    expectedOk: false,
    tags: ["ios", "semantic-error"],
    expectedErrorCode: "IOS_INCOMPLETE",
    expectedErrorIncludes: ["% Incomplete command"],
    postCommands: [],
    maxDurationMs: 10000,
  },
  {
    id: "ios-ambiguous",
    description: "comando ambiguo genera error",
    device: "ABEJITA",
    deviceKind: "ios",
    preCommands: ["enable"],
    command: "show i",
    expectedOk: false,
    tags: ["ios", "semantic-error"],
    expectedErrorCode: "IOS_AMBIGUOUS",
    expectedErrorIncludes: ["% Ambiguous command"],
    postCommands: [],
    maxDurationMs: 10000,
  },
  {
    id: "ios-wrong-mode",
    description: "show running-config desde user exec debe fallar",
    device: "ABEJITA",
    deviceKind: "ios",
    preCommands: ["disable"],
    command: "show running-config",
    expectedOk: false,
    tags: ["ios", "semantic-error", "wrong-mode"],
    expectedErrorCode: "IOS_INVALID_INPUT",
    expectedErrorIncludes: ["% Invalid input detected"],
    postCommands: ["enable"],
    maxDurationMs: 10000,
  },

  // ========================================================================
  // Host
  // ========================================================================
  {
    id: "host-ipconfig",
    description: "IPCONFIG /all para ver IP completa",
    device: "PC0",
    deviceKind: "host",
    command: "ipconfig /all",
    expectedOk: true,
    tags: ["host", "show"],
    expectedOutputIncludes: ["IP Address", "Subnet Mask"],
    maxDurationMs: 10000,
  },
  {
    id: "host-ping-success",
    description: "ping exitoso a localhost",
    device: "PC0",
    deviceKind: "host",
    command: "ping 127.0.0.1",
    expectedOk: true,
    tags: ["host", "ping"],
    expectedOutputIncludes: ["Success rate"],
    maxDurationMs: 15000,
  },
  {
    id: "host-ping-timeout",
    description: "ping timeout esperado (10.255.255.255 no alcanzable)",
    device: "PC0",
    deviceKind: "host",
    command: "ping 10.255.255.255",
    expectedOk: true,
    tags: ["host", "ping", "timeout-expected"],
    expectedOutputIncludes: ["Request timed out"],
    maxDurationMs: 15000,
  },
  {
    id: "host-invalid",
    description: "comando host invalido debe fallar semanticamente",
    device: "PC0",
    deviceKind: "host",
    command: "comando-invalido",
    expectedOk: false,
    tags: ["host", "semantic-error"],
    expectedErrorCode: "HOST_INVALID_COMMAND",
    expectedErrorIncludes: ["not recognized"],
    maxDurationMs: 10000,
  },

  // ========================================================================
  // Edge cases
  // ========================================================================
  {
    id: "ios-empty-output",
    description: "comando que devuelve output limpio sin errores",
    device: "ABEJITA",
    deviceKind: "ios",
    preCommands: ["enable"],
    command: "show processes cpu",
    expectedOk: true,
    tags: ["ios", "show", "short-output"],
    maxDurationMs: 10000,
  },
  {
    id: "ios-show-log",
    description: "show log con salida larga",
    device: "ABEJITA",
    deviceKind: "ios",
    preCommands: ["enable"],
    command: "show log",
    expectedOk: true,
    tags: ["ios", "show", "pager", "long-output"],
    maxDurationMs: 70000,
  },
];