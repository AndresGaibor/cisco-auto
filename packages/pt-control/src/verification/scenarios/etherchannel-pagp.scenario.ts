/**
 * Escenario EtherChannel con PAgP
 *
 * Topología: 2 switches conectados por 2 enlaces físicos que forman un Port-Channel.
 * Se configura PAgP (desirable/auto) y se verifica que el canal se establece correctamente.
...
export const etherchannelPagpScenario: RealScenarioDefinition = {
  id: "etherchannel-pagp",
  title: "EtherChannel PAgP Configuration",
  tags: ["etherchannel", "pagp", "switching"],
  ...
  async execute(ctx): Promise<{ outcome: ExecutionOutcome; evidence: Record<string, unknown>; warnings: string[]; error?: string }> {
    ...
    try {
      ...
      store.writeStepArtifact(ctx.runId, this.id, "execute", "pagp-config-commands.txt",
        "SW1: interface range FastEthernet0/2-3 > channel-group 1 mode desirable > Port-channel 1 trunk\n" +
        "SW2: interface range FastEthernet0/2-3 > channel-group 1 mode auto > Port-channel 1 trunk");

      return {
        outcome: "passed",
        evidence: { channelGroup: 1, mode: "desirable/auto", ports: ["FastEthernet0/2", "FastEthernet0/3"] },
        warnings,
      };
    } catch (e) {
      return {
