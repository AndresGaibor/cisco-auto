// ============================================================================
// Runtime recargable para Packet Tracer
// Contiene cola, heartbeat, dispatch y estado de negocio.
// ============================================================================

declare var ipc: any;

var IOS_JOBS: any = {};
var IOS_SESSIONS: any = {};
var commandEnded = true;
var outputWritten = true;
var modeChanged = true;
var moreDisplayed = false;

var CABLE_TYPES: Record<string, number> = {
...
var DEVICE_TYPES: Record<string, number> = {
  router: 0,
  switch: 1,
  cloud: 2,
  pc: 8,
...
var DEVICE_TYPE_NAMES: Record<number, string> = {
  0: "router",
  1: "switch",
  2: "cloud",
  8: "pc",
...
function getDeviceTypeString(typeId: number): string {
...
function getDeviceTypeCandidates(model: string): number[] {
...
function createDeviceWithFallback(host: any, model: string, x: number, y: number, typeList: number[]): any {
...
function getDevicePortNames(device: any): string[] {
  var names: string[] = [];
...
function resolveDevicePortName(device: any, requested: string): string | null {
  ...
  for (var i = 0; i < names.length; i++) {
    ...
    if (candidateValue === wanted) return String(names[i]);
  }
  var wantedKey = normalizePortKey(requested);
  for (var j = 0; j < names.length; j++) {
    var candidateName = names[j];
    if (candidateName && normalizePortKey(candidateName) === wantedKey) return String(candidateName);
  }
  return null;
}

function isEndDevice(device: any): boolean {
  try {
...
function getCableTypeId(name: string): number {
  var value = String(name || "straight").toLowerCase();
  if (value === "cross") return 8101;
  if (value === "roll") return 8102;
  if (value === "fiber") return 8103;
  if (value === "serial") return 8106;
  if (value === "console") return 8108;
  return 8100;
}

function getRuntimeState(host: any): any {
...
function getNet(host: any): any {
  return host.ipc.network();
}

function getLW(host: any): any {
...
function log(host: any, msg: string): void {
  host.log(msg);
}

function ensureDir(host: any, path: string): void {
  try {
...
function ensureRuntimeDirs(host: any): void {
  var state = getRuntimeState(host);
  if (state.dirsReady) return;
...
function getPendingCommandFiles(host: any): string[] {
  try {
...
function recoverInFlightOnStartup(host: any): void {
  reconcileInflight(host);
}

function loadPendingCommands(host: any): void {
  var state = getRuntimeState(host);
  var path = host.paths.PENDING_COMMANDS_FILE;
  if (!host.fm.fileExists(path)) return;
  try {
...
function savePendingCommands(host: any): void {
  var state = getRuntimeState(host);
  host.fm.writePlainTextToFile(host.paths.PENDING_COMMANDS_FILE, JSON.stringify(state.pendingCommands));
}

function writeHeartbeat(host: any): void {
  var state = getRuntimeState(host);
  var now = Date.now();
  if ((now - state.lastHeartbeatAt) < 5000) return;
...
function buildDevicePorts(device: any): any[] {
  var ports: any[] = [];
  var count = 0;
  try {
  ...
  for (var i = 0; i < count; i++) {
    try {
      var port = device.getPortAt(i);
      if (!port) continue;
      var portInfo: any = { name: port.getName ? String(port.getName()) : "" };
      try { portInfo.ipAddress = port.getIpAddress ? port.getIpAddress() : undefined; } catch (e1) {}
      try { portInfo.subnetMask = port.getSubnetMask ? port.getSubnetMask() : undefined; } catch (e2) {}
      try { portInfo.macAddress = port.getMacAddress ? port.getMacAddress() : undefined; } catch (e3) {}
      try { portInfo.defaultGateway = port.getDefaultGateway ? port.getDefaultGateway() : undefined; } catch (e4) {}
      try { portInfo.link = port.getLink ? port.getLink() : undefined; } catch (e5) {}
      ports.push(portInfo);
    } catch (e6) {}
  }
...
function collectSnapshot(host: any): any {
  var net = getNet(host);
  var count = 0;
  try {
  ...
  for (var i = 0; i < count; i++) {
    ...
    var name = "";
    try { name = String(device.getName ? device.getName() : ""); } catch (e2) { name = ""; }
    if (!name) continue;

    var typeId = 2;
    try { typeId = device.getType ? device.getType() : 2; } catch (e3) { typeId = 2; }
    ...
    for (var p = 0; p < ports.length; p++) {
      ...
      for (var j = 0; j < count; j++) {
        var other = null;
        try { other = net.getDeviceAt(j); } catch (e4) { other = null; }
        if (!other) continue;
        var otherName = "";
        try { otherName = String(other.getName ? other.getName() : ""); } catch (e5) { otherName = ""; }
        ...
        for (var op = 0; op < otherPorts.length; op++) {
          ...
          if (!otherPort || otherPort.link !== link) continue;
...
function handleSnapshot(host: any): any {
  try {
...
function handleInspect(payload: any, host: any): any {
  try {
    var net = getNet(host);
...
function handleMoveDevice(payload: any, host: any): any {
  try {
    var net = getNet(host);
    ...
    var moved = false;
    if (device.moveToLocation) moved = !!device.moveToLocation(x, y);
    if (!moved && device.moveToLocationCentered) moved = !!device.moveToLocationCentered(x, y);
    if (!moved) return { ok: false, error: "Packet Tracer rejected move", code: "INTERNAL_ERROR", details: { name: payload.name, x: x, y: y } };
...
function handleRenameDevice(payload: any, host: any): any {
  try {
    var net = getNet(host);
    var device = net.getDevice(payload.oldName || payload.name);
    if (!device) return { ok: false, error: "Device not found: " + (payload.oldName || payload.name) };
    if (device.setName) device.setName(payload.newName);
...
function handleAddModule(payload: any, host: any): any {
  try {
    var net = getNet(host);
    var device = net.getDevice(payload.device);
    if (!device) return { ok: false, error: "Device not found: " + payload.device };
    if (!device.addModule) return { ok: false, error: "Device does not support modules" };
...
function handleRemoveModule(payload: any, host: any): any {
  try {
    var net = getNet(host);
    var device = net.getDevice(payload.device);
    if (!device) return { ok: false, error: "Device not found: " + payload.device };
    if (!device.removeModule) return { ok: false, error: "Device does not support modules" };
...
function handleHardwareInfo(payload: any, host: any): any {
  try {
...
function handleHardwareCatalog(payload: any, host: any): any {
  try {
    var factory = host.ipc.hardwareFactory ? host.ipc.hardwareFactory() : null;
...
function handleCommandLog(payload: any, host: any): any {
  try {
    var cl = host.ipc.commandLog ? host.ipc.commandLog() : null;
    if (!cl) return { ok: false, error: "Command log unavailable" };
    ...
    for (var i = start; i < count; i++) {
      ...
      if (payload.device && entryDevice !== String(payload.device)) continue;
      entries.push({
        index: i,
...
function handleListCanvasRects(payload: any, host: any): any {
  try {
    var lw = getLW(host);
    var rectIds: any[] = [];
...
function handleGetRect(payload: any, host: any): any {
  try {
    var lw = getLW(host);
    var rectData = null;
...
function handleDevicesInRect(payload: any, host: any): any {
  try {
    var lw = getLW(host);
    var net = getNet(host);
    var devices: string[] = [];
...
function prepareIosSteps(device: any, command: string, ensurePrivileged: boolean): string[] {
  var steps: string[] = [];
  var prompt = "";
  try {
  ...
  if (!normalizedCommand) return steps;
...
function createIosJob(ticket: string, type: string, payload: any, host: any): any {
  var job = {
    ticket: ticket,
    type: type,
...
function getIosJobTerm(job: any): any {
  if (!job) return null;
...
function completeIosJob(ticket: string): void {
...
function failIosJob(ticket: string, message: string, code: string): void {
  var job = IOS_JOBS[ticket];
  if (!job) return;
  job.finished = true;
  job.state = "error";
...
function sendIosJobCommand(ticket: string, command: string, phase: string): void {
  var job = IOS_JOBS[ticket];
...
function advanceIosJob(ticket: string): void {
  var job = IOS_JOBS[ticket];
...
function getCommandResultId(record: any, seq: number): string {
  if (record && record.id) return String(record.id);
  return "cmd_" + padSeq(seq);
}

function claimNextCommand(host: any): any {
...
function writeResult(host: any, cmdId: string, result: any): void {
...
function removeInflightIfPresent(host: any, cmdId: string): void {
...
function reconcileInflight(host: any): void {
  var files = host.fm.getFilesInDirectory(host.paths.IN_FLIGHT_DIR);
  if (!files || files.length === 0) return;
  var now = Date.now();
  for (var i = 0; i < files.length; i++) {
    ...
    try {
      ...
      // Si el archivo está corrupto, intentamos eliminarlo para no bloquear la cola
      try {
        host.fm.removeFile(host.paths.IN_FLIGHT_DIR + "/" + filename);
      } catch (inner) {}
    }
...
function handleExecIos(payload: any, host: any): any {
  ...
  var ensurePrivileged = !!payload.options?.ensurePrivileged || command.toLowerCase().indexOf("show ") === 0;
...
function handleExecInteractive(payload: any, host: any): any {
  var ticket = "ios_job_" + Date.now() + "_" + Math.floor(Math.random() * 1000000);
  var net = getNet(host);
  var device = net.getDevice(payload.device);
  if (!device) return { ok: false, error: "Device not found: " + payload.device };
...
function handleAddDevice(payload: any, host: any): any {
  ...
  try {
    ...
    if (!created) return { ok: false, error: "Failed to add device for model: " + model };
...
function handleRemoveDevice(payload: any, host: any): any {
...
function handleListDevices(payload: any, host: any): any {
...
function handleAddLink(payload: any, host: any): any {
  var lw = host.ipc.appWindow().getActiveWorkspace().getLogicalWorkspace();
  var net = host.ipc.network();
  ...
  if (!first) {
    ...
    if (!second) return { ok: false, error: "Packet Tracer rejected the link" };
  }
  return { ok: true, device1: payload.device1, port1: port1, device2: payload.device2, port2: port2, cableType: cableType };
}

function handleRemoveLink(payload: any, host: any): any {
  try {
...
function handleConfigHost(payload: any, host: any): any {
  ...
  if (!device) return { ok: false, error: "Device not found: " + deviceName };
  try {
    var port = device.getPortAt ? device.getPortAt(0) : null;
    if (!port) return { ok: false, error: "No ports on device" };

    if (dhcp) {
      if (port.setDhcpEnabled) port.setDhcpEnabled(true);
    } else {
      if (ip && mask && port.setIpSubnetMask) port.setIpSubnetMask(ip, mask);
      if (gateway && port.setDefaultGateway) port.setDefaultGateway(gateway);
      if (dns && port.setDnsServerIp) port.setDnsServerIp(dns);
    }
...
function handleConfigIos(payload: any, host: any): any {
  var deviceName = payload.device;
...
function handleClearTopology(payload: any, host: any): any {
  try {
    ...
    for (var linkId in links) {
      if (!Object.prototype.hasOwnProperty.call(links, linkId)) continue;
      try {
    ...
    for (var deviceName in devices) {
      if (!Object.prototype.hasOwnProperty.call(devices, deviceName)) continue;
      try {
...
function handlePollDeferred(payload: any, host: any): any {
  ...
  if (!job.finished) return { done: false, state: job.state, phase: job.phase };
  if (job.state === "error") return { done: true, ok: false, error: job.error || "Job failed", raw: job.output || "" };
  return { done: true, ok: true, raw: job.output || "", status: job.status || 0 };
}

function dispatch(payload: any, host: any): any {
...
function processDeferred(host: any): void {
  var state = getRuntimeState(host);
...
function processNextCommand(host: any): void {
  var state = getRuntimeState(host);
...
function initRuntime(host: any): void {
  ensureRuntimeDirs(host);
  loadPendingCommands(host);
  recoverInFlightOnStartup(host);
}

function tickRuntime(host: any): void {
  if (host && host.isShuttingDown) return;
  ensureRuntimeDirs(host);
  reconcileInflight(host);
  writeHeartbeat(host);
...
function cleanupRuntime(host: any): void {
  if (host && host.isShuttingDown) return;
  savePendingCommands(host);
}
