...
describe("Link handlers (addLink/removeLink)", () => {
  test("handleAddLink returns error when device1 not found", () => {
    const net = { getDevice: (n: string) => n === "R1" ? { getName: () => "R1", skipBoot: () => {} } : null };
    const result = handleAddLink({ type: "addLink", device1: "NONEXISTENT", port1: "Gi0/0", device2: "R1", port2: "Gi0/0" }, createDeps({}, net, {}));
  ...
  test("handleAddLink returns error when device2 not found", () => {
    ...
    const result = handleAddLink({ type: "addLink", device1: "R1", port1: "Gi0/0", device2: "NONEXISTENT", port2: "Gi0/0" }, createDeps({}, net, {}));
    expect(result.ok).toBe(false);
    expect((result as any).code).toBe("DEVICE_NOT_FOUND");
  });

  test("handleAddLink calls createLink with swapped order when first is end device", () => {
    const pcPort = { getName: () => "Ethernet0", getPortCount: () => 1, getPortAt: () => null, getPort: () => pcPort };
    const routerPort = { getName: () => "GigabitEthernet0/0", getPortCount: () => 1, getPortAt: () => null, getPort: () => routerPort };
    const device = { getName: () => "PC1", skipBoot: () => {}, getType: () => 8, getPortCount: () => 1, getPortAt: () => pcPort, getPort: () => pcPort };
    const router = { getName: () => "R1", skipBoot: () => {}, getType: () => 0, getPortCount: () => 1, getPortAt: () => routerPort, getPort: () => routerPort };
    ...
    const result = handleAddLink({ type: "addLink", device1: "PC1", port1: "Ethernet0", device2: "R1", port2: "Gi0/0", linkType: "auto" }, createDeps(lw, net, fm));
  ...
  test("handleRemoveLink calls deleteLink", () => {
    let called = false;
    const lw = { deleteLink: () => { called = true; } };
    const fm = {
      fileExists: () => false,
    };
  ...
  test("handleRemoveLink cleans up registry entry", () => {
    ...
    const registryContent = { "R1:Gi0/0--SW1:Fa0/1": { device1: "R1", port1: "Gi0/0", device2: "SW1", port2: "Fa0/1" } };
    ...
    const fm = {
      ...
      writePlainTextToFile: (_: string, content: string) => { savedRegistry = JSON.parse(content); },
    };
