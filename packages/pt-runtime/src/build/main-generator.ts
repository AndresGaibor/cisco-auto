...
export function generateMainAsset(config: MainGeneratorConfig): GeneratedMainAsset {
  ...
  const sourceRoot = config.srcDir ?? path.join(process.cwd(), "src");
  ...
  const devDirLiteral = toPtRuntimePathLiteral(config.injectDevDir);
...
`;
  ...
  let fullCode = header + kernelIife + fileLoader + entryPoints;

  fullCode = sanitizeTypeScriptHelperGlobalThis(fullCode);

  if (process.env.DEBUG_MAIN_GEN) {
    fs.writeFileSync("/tmp/debug-main-generated.js", fullCode);
  }

  assertJavaScriptSyntaxOrThrow("main.js", fullCode);
