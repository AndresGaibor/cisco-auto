...
// Original: packages/pt-runtime/src/pt/kernel/output-completion-policy.ts
// FIX: PT usa self, no globalThis

const _g = typeof self !== "undefined" ? self : globalThis;
const _ios = (_g as any).IOS_DOMAIN?.kernel;

export function normalizeCommandForFallback(command: unknown): string {
  if (_ios?.normalizeCommandForFallback) {
    return _ios.normalizeCommandForFallback(command);
  }
  return String(command ?? "").trim().replace(/\s+/g, " ").toLowerCase();
}

export function isLongOutputReadOnlyIosCommand(command: unknown): boolean {
  if (_ios?.isLongOutputReadOnlyIosCommand) {
    return _ios.isLongOutputReadOnlyIosCommand(command);
  }
...
export function normalizeEol(value: unknown): string {
  if (_ios?.normalizeEol) {
    return _ios.normalizeEol(value);
  }
...
export function lastNonEmptyLine(value: unknown): string {
  if (_ios?.lastNonEmptyLine) {
    return _ios.lastNonEmptyLine(value);
  }
...
export function isPagerOnlyLine(line: string): boolean {
  if (_ios?.isPagerOnlyLine) {
    return _ios.isPagerOnlyLine(line);
  }
  return /^--More--$/i.test(String(line ?? "").trim());
}

export function lineContainsCommandEcho(line: string, command: string): boolean {
  if (_ios?.lineContainsCommandEcho) {
    return _ios.lineContainsCommandEcho(line, command);
  }
...
export function stripCommandEchoFromLine(line: string, command: string): string {
  if (_ios?.stripCommandEchoFromLine) {
    return _ios.stripCommandEchoFromLine(line, command);
  }
  const rawLine = String(line ?? "").trim();
  const rawCommand = String(command ?? "").trim();
...
export function isConfigPromptText(value: string): boolean {
  if (_ios?.isConfigPromptText) {
    return _ios.isConfigPromptText(value);
  }
  return /\(config[^)]*\)#\s*$/.test(String(value ?? "").trim());
}

export function isEndCommand(command: string): boolean {
  if (_ios?.isEndCommand) {
    return _ios.isEndCommand(command);
  }
  return String(command ?? "").trim().toLowerCase() === "end";
}

export function isPromptOnlyTransitionCommand(command: string): boolean {
  if (_ios?.isPromptOnlyTransitionCommand) {
    return _ios.isPromptOnlyTransitionCommand(command);
  }
...
export function blockHasCommandEcho(lines: string[], command: string): boolean {
  if (_ios?.blockHasCommandEcho) {
    return _ios.blockHasCommandEcho(lines, command);
  }
...
export function nativeEndCommandLooksComplete(lines: string[], command: string, prompt: string): boolean {
  if (_ios?.nativeEndCommandLooksComplete) {
    return _ios.nativeEndCommandLooksComplete(lines, command, prompt);
  }
...
export function nativePromptOnlyTransitionLooksComplete(
  ...
  if (_ios?.nativePromptOnlyTransitionLooksComplete) {
    return _ios.nativePromptOnlyTransitionLooksComplete(lines, command, prompt);
  }
...
export function nativeConfigCommandEchoAndPromptLooksComplete(
  ...
  if (_ios?.nativeConfigCommandEchoAndPromptLooksComplete) {
    return _ios.nativeConfigCommandEchoAndPromptLooksComplete(lines, command, prompt);
  }
  const promptLine = lastNonEmptyLine(lines.join("\n"));
...
export function outputHasPager(output: string): boolean {
  if (_ios?.outputHasPager) {
    return _ios.outputHasPager(output);
  }
...
export function nativeOutputTailHasActivePager(output: string): boolean {
  if (_ios?.nativeOutputTailHasActivePager) {
    return _ios.nativeOutputTailHasActivePager(output);
  }
...
export function outputLooksComplete(output: string, command: string): boolean {
  if (_ios?.outputLooksComplete) {
    return _ios.outputLooksComplete(output, command);
  }
...
export function normalizeIosMode(mode: unknown, prompt?: unknown): string {
  if (_ios?.normalizeIosMode) {
    return _ios.normalizeIosMode(mode, prompt);
  }
...
export function inferIosModeFromPrompt(prompt: unknown): string | null {
  if (_ios?.inferIosModeFromPrompt) {
    return _ios.inferIosModeFromPrompt(prompt);
  }
...
export function isConfigMode(mode: string | null | undefined, prompt?: unknown): boolean {
  if (_ios?.isConfigMode) {
    return _ios.isConfigMode(mode, prompt);
  }
...
export function inferPromptFromTerminalText(text: string): string {
  if (_ios?.inferPromptFromTerminalText) {
    return _ios.inferPromptFromTerminalText(text);
  }
...
export function firstMeaningfulNativeOutputLine(output: unknown, command?: string): string {
  if (_ios?.firstMeaningfulNativeOutputLine) {
    return _ios.firstMeaningfulNativeOutputLine(output, command);
  }
...
export function lineLooksLikeNativeInterfaceHeader(line: string): boolean {
  if (_ios?.lineLooksLikeNativeInterfaceHeader) {
    return _ios.lineLooksLikeNativeInterfaceHeader(line);
  }
...
export function nativeLongOutputLooksPartial(args: {
  ...
  if (_ios?.nativeLongOutputLooksPartial) {
    return _ios.nativeLongOutputLooksPartial(args);
  }
...
export function buildNativeLongOutputWarnings(args: {
  ...
  if (_ios?.buildNativeLongOutputWarnings) {
    return _ios.buildNativeLongOutputWarnings(args);
  }
...
export function nativeLongOutputCanCompleteWithoutEcho(args: { block: string; command: string; prompt: string }): boolean {
  if (_ios?.nativeLongOutputCanCompleteWithoutEcho) {
    return _ios.nativeLongOutputCanCompleteWithoutEcho(args);
  }
...
export function nativeLongOutputHasCommandEvidence(command: string, block: string): boolean {
  if (_ios?.nativeLongOutputHasCommandEvidence) {
    return _ios.nativeLongOutputHasCommandEvidence(command, block);
  }
  const cmd = normalizeCommandForFallback(command);
...
export function detectIosSemanticErrorFromOutput(output: unknown): { code: string; message: string } | null {
  if (_ios?.detectIosSemanticErrorFromOutput) {
    return _ios.detectIosSemanticErrorFromOutput(output);
  }
...
export function isIosConfigPromptText(value: unknown): boolean {
  if (_ios?.isIosConfigPromptText) {
    return _ios.isIosConfigPromptText(value);
  }
...
export function isIosConfigModeText(value: unknown): boolean {
  if (_ios?.isIosConfigModeText) {
    return _ios.isIosConfigModeText(value);
  }
...
export function nativeSnapshotIsStillInConfigMode(snapshot: { prompt?: unknown; mode?: unknown }): boolean {
  if (_ios?.nativeSnapshotIsStillInConfigMode) {
    return _ios.nativeSnapshotIsStillInConfigMode(snapshot);
  }
...
export function inferModeFromPrompt(prompt: string): string {
  if (_ios?.inferModeFromPrompt) {
    return _ios.inferModeFromPrompt(prompt);
  }
  return normalizeIosMode("unknown", prompt);
}

export function extractLatestCommandBlock(output: string, command: string): string {
  if (_ios?.extractLatestCommandBlock) {
    return _ios.extractLatestCommandBlock(output, command);
  }
  const text = normalizeEol(output);
  const cmd = String(command || "").trim();
...
export function extractCurrentCommandBlockStrict(
  ...
  if (_ios?.extractCurrentCommandBlockStrict) {
    return _ios.extractCurrentCommandBlockStrict(output, command);
  }
  const text = normalizeEol(output);
  const cmd = String(command || "").trim();
  if (!text.trim() || !cmd) {
...
export function appendStepOutput(current: string, next: unknown): string {
  if (_ios?.appendStepOutput) {
    return _ios.appendStepOutput(current, next);
  }
