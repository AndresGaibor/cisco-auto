...
var KNOWN_CAPABILITIES: Record<string, { sinceVersion: string; fallback: string }> = {
  ...
  "PTDevice.moveToLocation": { sinceVersion: "7.3", fallback: "Use moveToLocationCentered instead" },
  "PTDevice.moveToLocationCentered": { sinceVersion: "7.3", fallback: "Use moveToLocation instead" },
  ...
  "PTPort.getInboundFirewallServiceStatus": { sinceVersion: "8.0", fallback: "No firewall status API" },
  ...
  "PTProcess.getDhcpServerProcessByPortName": { sinceVersion: "7.3", fallback: "No DHCP Server API support" },
  "PTProcess.getVlanCount": { sinceVersion: "7.3", fallback: "No VLAN API support — use IOS commands" },
};
...
function makeVersion(major: number, minor: number, patch: number): PtVersion {
  return {
    ...
    minor: minor,
    patch: patch,
    toString: function() { return major + "." + minor + "." + patch; },
  };
}
...
export function probeCapabilities(ipc: PtIpc): Record<string, boolean> {
  ...
  try {
    ...
    var sampleDevice = deviceCount > 0 ? net.getDeviceAt(0) : null;
    ...
    if (sampleDevice) {
      ...
      results["PTDevice.moveToLocationCentered"] = typeof sampleDevice.moveToLocationCentered === "function";
      ...
      if (portCount > 0) {
        ...
        if (port) {
          results["PTPort.setDhcpClientFlag"] = typeof (port as any).setDhcpClientFlag === "function";
          ...
          results["PTPort.getInboundFirewallServiceStatus"] = typeof (port as any).getInboundFirewallServiceStatus === "function";
        }
      }
      ...
      try {
        var process = (sampleDevice as any).getProcess ? (sampleDevice as any).getProcess("dhcpserver") : null;
        if (process) {
          results["PTProcess.getDhcpServerProcessByPortName"] = typeof (process as any).getDhcpServerProcessByPortName === "function";
        }
      } catch (e) { /* DHCP process not available */ }
      try {
        var vlanProcess = (sampleDevice as any).getProcess ? (sampleDevice as any).getProcess("vlan") : null;
        if (vlanProcess) {
          results["PTProcess.getVlanCount"] = typeof (vlanProcess as any).getVlanCount === "function";
        }
      } catch (e) { /* VLAN process not available */ }
    }

    try {
      var lw = ipc.appWindow()?.getActiveWorkspace()?.getLogicalWorkspace();
      results["PTLogicalWorkspace.deleteDevice"] = lw ? typeof (lw as any).deleteDevice === "function" : false;
      results["PTLogicalWorkspace.deleteObject"] = lw ? typeof (lw as any).deleteObject === "function" : false;
    } catch (e) {
...
export class PtSafeCall {
  ...
  constructor(capabilities: Record<string, boolean>, logger: ReturnType<typeof getLogger>) {
  ...
  call(
    ...
    if (fallback) {
      this.logger.debug("Capability not available, using fallback", { method: methodName });
      return fallback();
    }

    this.logger.warn("Capability not available and no fallback", { method: methodName });
    return undefined;
  }
}
