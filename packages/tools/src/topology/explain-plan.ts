...
/**
 * Resultado de la explicación del plan
 */
...
interface PlanTexts {
  ...
  conexiones: string;
  esquemaIp: string;
  protocoloRouting: string;
  recomendacion: string;
}

const TEXTS_ES: PlanTexts = {
  overview: 'Resumen',
 ...
 */
function getTexts(language: 'es' | 'en'): PlanTexts {
 ...
 */
const NETWORK_TYPE_TEXT: Record<NetworkType, { es: string; en: string }> = {
 ...
 */
const ROUTING_PROTOCOL_TEXT: Record<RoutingProtocol, { es: string; en: string }> = {
 ...
 */
const CABLE_TYPE_TEXT: Record<string, { es: string; en: string }> = {
 ...
 */
function generateOverview(plan: TopologyPlan, texts: PlanTexts, language: 'es' | 'en'): ExplanationSection {
 ...
 */
function generateDevicesSection(plan: TopologyPlan, texts: PlanTexts, language: 'es' | 'en'): ExplanationSection {
 ...
 */
function generateConnectionsSection(plan: TopologyPlan, texts: PlanTexts, language: 'es' | 'en'): ExplanationSection {
 ...
 */
function generateIPSchemeSection(plan: TopologyPlan, texts: PlanTexts, language: 'es' | 'en'): ExplanationSection {
  const allIps: Array<{ device: string; interface: string; ip: string; mask: string }> = [];
 ...
 */
function generateRoutingSection(plan: TopologyPlan, texts: PlanTexts, language: 'es' | 'en'): ExplanationSection {
 ...
 */
function generateRecommendationsSection(plan: TopologyPlan, texts: PlanTexts, language: 'es' | 'en'): ExplanationSection {
 ...
 */
function explainTopologyPlan(plan: TopologyPlan, language: 'es' | 'en'): ExplainPlanResult {
...
export const ptExplainPlanTool: Tool = {
  ...
  handler: async (input: ToolInput): Promise<ToolResult<ExplainPlanResult>> => {
    const plan = input.plan as TopologyPlan;
    const language = (input.language as 'es' | 'en') || 'es';
