...
export function createLinkId(device1: string, port1: string, device2: string, port2: string): string {
...
export function createEmptySnapshot(): TopologySnapshot {
  return {
    version: '1.0',
    timestamp: Date.now(),
    devices: {},
...
export function calculateDelta(from: TopologySnapshot, to: TopologySnapshot): TopologyDelta {
  const deviceDeltas: DeviceDelta[] = [];
    ...
    if (!fromDevice && toDevice) {
      ...
      for (const key of allKeys) {
        if (JSON.stringify(fromDevice[key as keyof DeviceState]) !==
          ...
          changes[key] = toDevice[key as keyof DeviceState];
        }
      }
