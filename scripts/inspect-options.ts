...
async function rosettaOptions() {
  ...
  try {
    ...
    console.log("\n🛰️  ESCANEANDO MÉTODOS DE ipc.options()...");
    ...
    for (let i = 0; i < methodList.length; i++) {
        ...
        const code = `
            (function() {
                try {
                    var opt = ipc.options();
                    if (typeof opt['${m}'] === "function") {
                        try { return "LIVE() -> " + String(opt['${m}']()).substring(0, 30); }
                        catch(e) { return "LIVE(args)"; }
                    }
                } catch(e) {}
                return null;
            })()
        ...
        if (res.ok && res.result) {
            console.log(`  \x1b[32m${m}\x1b[0m: ${res.result}`);
        }
    }
  } catch (error: any) {
